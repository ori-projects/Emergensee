...
class MaternalProcessor(UnLabeledProcessor):
    ...
    def predict_models(self, models, maternal_row):
        """
        Predict risk levels using the trained models for a given row of Maternal data.

        Args:
        models (list): List of model instances.
        maternal_row (array-like): Row of data to predict.
        ...
        for model in models:
            prediction = model.predict(maternal_row)
    ...
    def get_risk_level_value(self, risk_level):
        ...
        """
        if risk_level == 'RiskLevel_low_risk':
            return 0
        elif risk_level == 'RiskLevel_mid_risk':
            return 50
        else:
            return 100
